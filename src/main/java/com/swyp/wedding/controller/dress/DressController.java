...
@Tag(name = "드레스", description = "웨딩 드레스 관리 API")
@RestController
...
public class DressController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<DressResponse>>> getAllDresses(
    ...
    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<DressResponse>> getDressById(
            @Parameter(description = "드레스 ID", required = true) @PathVariable Long id) {
        DressResponse dress = dressService.getDressById(id);
        return ResponseEntity.ok(ApiResponse.success(dress));
    }

    @Operation(summary = "새 드레스 등록", description = "새로운 드레스를 등록합니다. shop_name은 필수입니다.")
    @PostMapping
    public ResponseEntity<ApiResponse<DressResponse>> createDress(@RequestBody DressRequest request) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<DressResponse>> updateDress(
            @Parameter(description = "드레스 ID", required = true) @PathVariable Long id,
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<ApiResponse<Void>> deleteDress(
            @Parameter(description = "드레스 ID", required = true) @PathVariable Long id) {
